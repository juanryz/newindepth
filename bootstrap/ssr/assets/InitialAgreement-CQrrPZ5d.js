...
function InitialAgreement({ userAge }) {
  ...
  const CheckboxItem = ({ id, label, checked, onChange }) => /* @__PURE__ */ jsxs("label", { className: `flex items-start gap-3 p-4 rounded-xl border cursor-pointer transition-all ${checked ? "border-indigo-500 bg-indigo-50 dark:bg-indigo-900/30 text-indigo-900 dark:text-indigo-100" : "border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800/40 hover:border-indigo-300 dark:hover:border-indigo-600"}`, children: [
    /* @__PURE__ */ jsx("input", { type: "checkbox", id, checked, onChange: (e) => onChange(e.target.checked), className: "mt-1 w-5 h-5 accent-indigo-600 rounded text-indigo-600 focus:ring-indigo-500" }),
    /* @__PURE__ */ jsx("span", { className: `text-sm leading-relaxed ${checked ? "font-bold" : "text-gray-700 dark:text-gray-300"}`, children: label })
  ] });
