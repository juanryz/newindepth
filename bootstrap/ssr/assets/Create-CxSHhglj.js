          ...
          step === 2 && /* @__PURE__ */ jsxs("div", { className: "space-y-8 animate-fade-in-up", children: [
            /* @__PURE__ */ jsxs("div", { className: "bg-white/60 dark:bg-gray-800/60 backdrop-blur-xl shadow-sm sm:rounded-2xl border border-gray-100 dark:border-gray-700/50 overflow-hidden", children: [
              ...
              /* @__PURE__ */ jsxs("div", { className: "p-6", children: [
                ...
                /* @__PURE__ */ jsx("div", { className: "flex overflow-x-auto pb-6 -mx-6 px-6 gap-4 snap-x snap-mandatory lg:mx-0 lg:px-0 lg:grid lg:grid-cols-3 lg:gap-6 no-scrollbar", children: Object.values(packageOptions.packages).map((pkg) => {
                  const isRecommended = packageOptions.recommended === pkg.id && !packageOptions.is_vip_only;
                  return /* @__PURE__ */ jsxs(
                    "div",
                    {
                      className: `relative min-w-[85%] sm:min-w-[45%] lg:min-w-0 snap-center rounded-2xl border-2 p-5 sm:p-6 transition-all duration-300 cursor-pointer flex flex-col ${data.package_type === pkg.id ? "border-gold-500 bg-gold-50/50 dark:bg-gold-900/30 shadow-lg ring-1 ring-gold-500/20" : packageOptions.is_vip_only && pkg.id !== "vip" ? "opacity-40 grayscale cursor-not-allowed border-gray-100 dark:border-gray-800" : "border-gray-100 dark:border-gray-800 bg-white/40 dark:bg-gray-800/40 hover:border-gold-300"}`,
                      onClick: () => (!packageOptions.is_vip_only || pkg.id === "vip") && setData("package_type", pkg.id),
                      children: [
                        isRecommended && /* @__PURE__ */ jsx("div", { className: "absolute -top-3 left-1/2 -translate-x-1/2 bg-indigo-600 text-white text-[9px] font-black px-3 py-1 rounded-full uppercase tracking-widest shadow-lg z-10 whitespace-nowrap", children: "Rekomendasi" }),
                        /* @__PURE__ */ jsxs("div", { className: "flex justify-between items-start mb-3", children: [
                          /* @__PURE__ */ jsx("h4", { className: "font-black text-xs sm:text-sm uppercase tracking-widest text-gray-900 dark:text-white", children: pkg.name }),
                          /* @__PURE__ */ jsx("div", { className: `w-5 h-5 rounded-full border-2 flex items-center justify-center transition-colors ${data.package_type === pkg.id ? "border-gold-500 bg-gold-500" : "border-gray-300"}`, children: data.package_type === pkg.id && /* @__PURE__ */ jsx("div", { className: "w-2 h-2 rounded-full bg-white text-[10px] flex items-center justify-center" }) })
                        ] }),
                        /* @__PURE__ */ jsxs("div", { className: "mb-4", children: [
                          pkg.original_price && /* @__PURE__ */ jsxs("div", { className: "text-[10px] text-gray-400 dark:text-gray-500 line-through mb-0.5", children: [
                            "Rp ",
                            new Intl.NumberFormat("id-ID").format(pkg.original_price)
                          ] }),
                          /* @__PURE__ */ jsxs("p", { className: "text-xl sm:text-2xl font-black text-gold-600 dark:text-gold-400 leading-none", children: [
                            "Rp ",
                            new Intl.NumberFormat("id-ID").format(pkg.price)
                          ] }),
                          /* @__PURE__ */ jsx("p", { className: "text-[8px] sm:text-[9px] font-bold text-gray-400 dark:text-gray-500 mt-1 uppercase tracking-widest", children: "*Belum termasuk PPN 11%" })
                        ] }),
                        /* @__PURE__ */ jsx("div", { className: "text-[10px] sm:text-[11px] font-medium text-gray-600 dark:text-gray-200 leading-relaxed prose prose-sm dark:prose-invert max-w-none flex-grow", dangerouslySetInnerHTML: { __html: pkg.description } })
                      ]
                    },
                    pkg.id
                  );
