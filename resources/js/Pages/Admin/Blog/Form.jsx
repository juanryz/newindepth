...
export default function BlogForm({ post, seoRules, forbiddenWords = [] }) {
    ...
    useEffect(() => {
        const runAnalysis = async () => {
            ...
            try {
                ...
                console.error("Analisis SEO gagal", err);
            } finally {
    ...
    useEffect(() => {
        ...
        const found = forbiddenWords.filter(word => allText.includes(word.toLowerCase()));
        setFoundForbidden(found);
    ...
    const handleGenerate = async (type = 'full') => {
        ...
        setGenError('');
        if (type === 'full') setGenProgress('Menggenerate artikel lengkap (~60-90 detik)...');
        else if (type === 'meta') setGenProgress('Menggenerate meta...');
        else if (type === 'ideas') setGenProgress('Mencari ide artikel...');
        ...
        try {
            ...
            if (type === 'full') {
                ...
                setGenProgress('✅ Artikel berhasil digenerate! Pastikan review konten sebelum publish.');
            } else if (type === 'meta') {
                if (result.seo_title) setData('meta_title', result.seo_title);
                if (result.meta_description) setData('meta_description', result.meta_description);
                ...
                setGenProgress('✅ Ide artikel ditemukan!');
            }
    ...
    const getWordCount = () => {
        ...
        return data.body.replace(/<[^>]+>/g, '').split(/\s+/).filter(w => w.length > 0).length;
    };
                            ...
                            <div className="flex items-start gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs text-red-600 dark:text-red-400 mt-1">
                                        Kata berikut harus dihindari untuk menjaga kualitas dan etika konten:
                                    </p>
                                    <div className="flex flex-wrap gap-2 mt-2">
                                        {foundForbidden.map((word, i) => (
                                            <span key={i} className="px-2 py-1 bg-red-100 dark:bg-red-900/40 text-red-700 dark:text-red-300 text-[10px] font-black uppercase rounded-lg">
                                                {word}
                                            </span>
                                        ))}
                                    </div>
                            ...
                            <div className="p-8 md:p-10">
                                ...
                                <div className="bg-white/60 dark:bg-gray-800/60 rounded-2xl p-5 mb-6 border border-gray-200/50 dark:border-gray-700/50">
                                    <div className="flex items-center justify-between mb-3">
                                        <p className="text-[10px] font-black uppercase tracking-widest text-amber-600 dark:text-amber-400">💡 Cari Ide Artikel</p>
                                    </div>
                                    ...
                                    <div className="flex gap-3">
                                        <TextInput
                                            ...
                                            value={ideasKeyword}
                                            onChange={(e) => setIdeasKeyword(e.target.value)}
                                            placeholder="Masukkan topik, misal: hipnoterapi, kecemasan, trauma..."
                                        />
                                        <button
                                            onClick={() => handleGenerate('ideas')}
                                            disabled={isGenerating}
                                            className="px-6 py-2 bg-amber-500 hover:bg-amber-600 text-white text-[10px] font-black uppercase tracking-widest rounded-xl transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-lg shadow-amber-500/20"
                                        >
                                            {isGenerating ? '⏳' : '💡 Cari Ide'}
                                        </button>
                                    </div>
                                    ...
                                    {showIdeas && ideas.length > 0 && (
                                        <div className="mt-4 space-y-2">
                                            <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">📋 {ideas.length} Ide Artikel Ditemukan</p>
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                                {ideas.map((idea, i) => (
                                                    <div key={i} className="bg-white dark:bg-gray-900 p-4 rounded-2xl border border-gray-100 dark:border-gray-700 hover:border-indigo-300 dark:hover:border-indigo-600 transition-all group cursor-pointer"
                                                        ...
                                                        <div className="flex items-center gap-2 mt-3">
                                                            ...
                                                            <span className={`px-2 py-0.5 text-[9px] font-bold uppercase rounded-md ${idea.volume === 'tinggi' ? 'bg-emerald-50 dark:bg-emerald-950/40 text-emerald-600' :
                                                                    idea.volume === 'sedang' ? 'bg-amber-50 dark:bg-amber-950/40 text-amber-600' :
                                                                        'bg-gray-50 dark:bg-gray-800 text-gray-500'
                                                                }`}>📊 {idea.volume}</span>
                                                            <span className="px-2 py-0.5 bg-purple-50 dark:bg-purple-950/40 text-purple-600 dark:text-purple-400 text-[9px] font-bold uppercase rounded-md">{idea.intent}</span>
                                                        </div>
                                                        <p className="text-[9px] text-indigo-500 font-bold mt-2 opacity-0 group-hover:opacity-100 transition-opacity">Klik untuk gunakan →</p>
                                                    </div>
                                                ))}
                                            </div>
                                        </div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                                    <div>
                                        ...
                                        <TextInput className="w-full font-bold text-lg" value={genKeyword} onChange={(e) => setGenKeyword(e.target.value)} placeholder="Contoh: hipnoterapi" />
                                    </div>
                                    <div>
                                        ...
                                        <TextInput className="w-full text-sm" value={genSecondary} onChange={(e) => setGenSecondary(e.target.value)} placeholder="terapi hipnosis, terapi pikiran bawah sadar" />
                                    </div>
                                    <div>
                                        <InputLabel value="Gaya Bahasa" className="text-[10px] font-black uppercase tracking-widest text-indigo-600 dark:text-indigo-400 mb-2" />
                                    ...
                                    <div>
                                        <InputLabel value="Target Pembaca" className="text-[10px] font-black uppercase tracking-widest text-indigo-600 dark:text-indigo-400 mb-2" />
                                ...
                                {/* Ringkasan Aturan SEO Aktif */}
                                {seoRules && (
                                    <div className="bg-white/50 dark:bg-gray-800/50 rounded-2xl p-4 mb-6 border border-gray-200/50 dark:border-gray-700/50">
                                        <p className="text-[10px] font-black uppercase tracking-widest text-gray-400 mb-3">📋 Aturan SEO yang Aktif</p>
                                        <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-6 gap-3 text-center">
                                            {[
                                                { val: getRule('article_ideal_words', 2000), label: 'kata min' },
                                                { val: `${getRule('h2_min_count', 5)}-${getRule('h2_max_count', 8)}`, label: 'heading H2' },
                                                { val: `${getRule('keyword_density_min', 1)}-${getRule('keyword_density_max', 1.5)}%`, label: 'density' },
                                                { val: `${getRule('faq_min_questions', 4)}-${getRule('faq_ideal_questions', 5)}`, label: 'FAQ' },
                                                { val: `${getRule('internal_link_min', 3)}-${getRule('internal_link_ideal', 5)}`, label: 'int. link' },
                                                { val: `${getRule('image_min_count', 2)}-${getRule('image_ideal_count', 3)}`, label: 'gambar' },
                                            ].map((item, i) => (
                                                <div key={i} className="bg-white dark:bg-gray-900 rounded-xl p-2 border border-gray-100 dark:border-gray-700">
                                                    <p className="text-lg font-black text-indigo-600">{item.val}</p>
                                ...
                                {genError && (
                                    <div className="p-4 mb-4 text-sm text-red-800 dark:text-red-300 rounded-2xl bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800/50">❌ {genError}</div>
                                )}
                                {genProgress && (
                                    <div className="p-4 mb-4 text-sm text-indigo-800 dark:text-indigo-300 rounded-2xl bg-indigo-50 dark:bg-indigo-900/30 border border-indigo-200 dark:border-indigo-800/50">
                                        {isGenerating && (<div className="flex gap-1 mb-2"><span className="w-1.5 h-1.5 bg-indigo-500 rounded-full animate-bounce"></span><span className="w-1.5 h-1.5 bg-indigo-500 rounded-full animate-bounce" style={{ animationDelay: '0.1s' }}></span><span className="w-1.5 h-1.5 bg-indigo-500 rounded-full animate-bounce" style={{ animationDelay: '0.2s' }}></span></div>)}
                                        {genProgress}
                                    </div>
                                )}

                                <div className="flex flex-wrap gap-3">
                                    <button onClick={() => handleGenerate('full')} disabled={isGenerating}
                                    ...
                                    <button onClick={() => handleGenerate('meta')} disabled={isGenerating}
                        ...
                        <div className="lg:col-span-8 space-y-6">
                            <div className="bg-white dark:bg-gray-800 overflow-hidden shadow-2xl sm:rounded-[2.5rem] border border-gray-100 dark:border-gray-700/50 p-8 md:p-10">
                                <form onSubmit={submit} className="space-y-8">
                                    ...
                                    <div className="space-y-4">
                                        <div>
                                            <div className="flex items-center justify-between mb-2">
                                                ...
                                                <span className={`text-[10px] font-bold ${data.title.length >= getRule('h1_min_length', 60) && data.title.length <= getRule('h1_max_length', 80) ? 'text-emerald-500' : data.title.length > getRule('h1_max_length', 80) ? 'text-red-500' : 'text-gray-400'}`}>
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                            <div>
                                                <InputLabel htmlFor="primary_keyword" value="Keyword Utama" className="text-[10px] font-black uppercase tracking-widest text-gray-400 mb-2" />
                                                <TextInput id="primary_keyword" className="w-full font-bold" value={data.primary_keyword} onChange={(e) => setData('primary_keyword', e.target.value)} placeholder="Contoh: hipnoterapi trauma" />
                                            </div>
                                            <div>
                                                <InputLabel htmlFor="secondary_keywords" value="Keyword Sekunder (Komma)" className="text-[10px] font-black uppercase tracking-widest text-gray-400 mb-2" />
                                                <TextInput id="secondary_keywords" className="w-full text-xs" value={data.secondary_keywords} onChange={(e) => setData('secondary_keywords', e.target.value)} placeholder="Contoh: trauma psikologis, cara kerja hipnoterapi" />
                                            </div>
                                            <div>
                                                <InputLabel value="Intensi Pencarian" className="text-[10px] font-black uppercase tracking-widest text-gray-400 mb-2" />
                                                <div className="px-4 py-2.5 bg-indigo-50 dark:bg-indigo-950/40 text-indigo-700 dark:text-indigo-300 rounded-xl text-xs font-black inline-block min-w-full md:min-w-0">{intent}</div>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    {/* RINGKASAN */}
                                    <div>
                                        <InputLabel htmlFor="excerpt" value="Ringkasan Singkat" className="text-[10px] font-black uppercase tracking-widest text-gray-400 mb-2" />
                                        <textarea id="excerpt"
                                            ...
                                            value={data.excerpt} onChange={(e) => setData('excerpt', e.target.value)}
                                    ...
                                    <div>
                                        <div className="flex items-center justify-between mb-2">
                                            ...
                                            <span className={`text-[10px] font-bold ${getWordCount() >= getRule('article_min_words', 1800) ? 'text-emerald-500' : 'text-gray-400'}`}>
                                                {getWordCount()} / {getRule('article_min_words', 1800)}+ kata
                                            </span>
                                        </div>
                                        <div className="mt-1 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-2xl overflow-hidden pb-14 group transition-all focus-within:ring-2 focus-within:ring-indigo-500/20">
                                            <style>{`
                                                .dark .ql-editor { color: #e2e8f0; font-size: 1rem; line-height: 1.7; }
                                                .dark .ql-editor.ql-blank::before { color: #64748b; }
                                                .dark .ql-toolbar { border-bottom: 1px solid #334155 !important; background: #0f172a; }
                                                .dark .ql-container { border: none !important; }
                                                .dark .ql-snow .ql-stroke { stroke: #94a3b8; }
                                                .dark .ql-snow .ql-fill { fill: #94a3b8; }
                                                .dark .ql-snow .ql-picker { color: #94a3b8; }
                                                .dark .ql-snow .ql-picker-options { background-color: #1e293b; border: 1px solid #334155; }
                                                .ql-editor { min-height: 450px; font-family: inherit; }
                                                .ql-toolbar.ql-snow { border: none !important; padding: 12px; }
                                                .ql-editor img { max-width: 100%; height: auto; border-radius: 12px; margin: 16px 0; }
                                            `}</style>
                                            <ReactQuill theme="snow" value={data.body} onChange={(content) => setData('body', content)} className="h-[500px]"
                                                modules={{
                                                    toolbar: [
                                                        [{ 'header': [1, 2, 3, false] }],
                                        ...
                                        <div className="space-y-4">
                                            ...
                                            <div className="relative group p-6 border-2 border-dashed border-gray-200 dark:border-gray-700 rounded-3xl hover:border-indigo-500/50 transition-all text-center">
                                                {isEditing && post.featured_image && !data.featured_image && (
                                                    <img src={`/storage/${post.featured_image}`} alt="Gambar saat ini" className="h-32 mx-auto rounded-2xl mb-4 shadow-lg" />
                                                )}
                                                <input id="featured_image" type="file" accept="image/*" className="absolute inset-0 opacity-0 cursor-pointer"
                                                ...
                                                <div className="text-gray-400 group-hover:text-indigo-500 transition-colors">
                                                    ...
                                                    <p className="text-xs font-black uppercase tracking-widest">{data.featured_image ? data.featured_image.name : 'Unggah Gambar Utama'}</p>
                                                </div>
                                            </div>
                                        </div>
                                        <div className="space-y-4">
                                            ...
                                            <div>
                                                <div className="flex items-center justify-between mb-1">
                                                    <span className="text-[10px] font-bold text-gray-400">Judul SEO</span>
                                                    <span className={`text-[10px] font-bold ${data.meta_title.length >= getRule('seo_title_min_length', 55) && data.meta_title.length <= getRule('seo_title_max_length', 65) ? 'text-emerald-500' : 'text-gray-400'}`}>
                                            ...
                                            <div>
                                                <div className="flex items-center justify-between mb-1">
                                                    <span className="text-[10px] font-bold text-gray-400">Deskripsi Meta</span>
                                                    <span className={`text-[10px] font-bold ${data.meta_description.length >= getRule('meta_desc_min_length', 140) && data.meta_description.length <= getRule('meta_desc_max_length', 160) ? 'text-emerald-500' : 'text-gray-400'}`}>
                                                        {data.meta_description.length}/{getRule('meta_desc_min_length', 140)}-{getRule('meta_desc_max_length', 160)}
                                                    </span>
                                                </div>
                                                <textarea className="w-full bg-gray-50 dark:bg-gray-900 border-none rounded-xl focus:ring-2 focus:ring-indigo-500/20 dark:text-gray-300 text-xs font-medium min-h-[100px]"
                                                    value={data.meta_description} onChange={(e) => setData('meta_description', e.target.value)} placeholder="Deskripsi halaman untuk mesin pencari..." />
                                            </div>
                                        </div>
                                    </div>
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                            <label className="flex items-center gap-3 cursor-pointer group">
                                                <div className="relative">
                                                    <input type="checkbox" className="sr-only peer" checked={data.is_published}
                                                        onChange={(e) => { setData('is_published', e.target.checked); if (e.target.checked) setData('scheduled_at', ''); }} />
                                                    <div className="w-11 h-6 bg-gray-200 peer-focus:outline-none rounded-full peer dark:bg-gray-700 peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:start-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all dark:border-gray-600 peer-checked:bg-indigo-600"></div>
                                                </div>
                                                <span className="text-sm font-black text-gray-500 group-hover:text-gray-900 dark:group-hover:text-white transition-colors uppercase tracking-tight">
                                            ...
                                            <div>
                                                <div className="flex items-center gap-2 mb-2">
                                                    ...
                                                    <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Jadwalkan Publikasi</span>
                                                </div>
                                                <input type="datetime-local"
                                                    className={`w-full bg-gray-50 dark:bg-gray-900 border rounded-xl text-sm font-medium focus:ring-2 focus:ring-indigo-500/20 dark:text-gray-300 transition-all ${data.scheduled_at ? 'border-indigo-300 dark:border-indigo-700' : 'border-gray-200 dark:border-gray-700'
                                                        }`}
                                                    value={data.scheduled_at}
                                                    onChange={(e) => { setData('scheduled_at', e.target.value); if (e.target.value) setData('is_published', false); }}
                                                    disabled={data.is_published}
                                                    min={new Date().toISOString().slice(0, 16)}
                                                />
                                                {data.scheduled_at && !data.is_published && (
                                                    <p className="text-[10px] text-indigo-500 dark:text-indigo-400 font-bold mt-2">
                                                        📅 Artikel akan dipublikasikan pada {new Date(data.scheduled_at).toLocaleDateString('id-ID', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })} pukul {new Date(data.scheduled_at).toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' })} WIB
                                                    </p>
                                                )}
                                            </div>
                                        </div>
                                    ...
                                    <div className="flex items-center justify-end pt-6 border-t border-gray-100 dark:border-gray-700/50">
                                        <PrimaryButton className="px-10 py-4 !rounded-2xl shadow-xl shadow-indigo-500/20" disabled={processing}>
                                            {data.is_published ? (isEditing ? 'Simpan & Publikasikan' : 'Publikasikan Sekarang') :
                                                data.scheduled_at ? 'Jadwalkan Publikasi' :
                        ...
                        <div className="lg:col-span-4 space-y-6 sticky top-24">
                            ...
                            <div className="bg-white dark:bg-gray-800 rounded-[2rem] shadow-2xl overflow-hidden border border-gray-100 dark:border-gray-700/50 p-5">
                                ...
                                <div className="flex flex-wrap gap-1">
                                    {forbiddenWords.slice(0, 12).map((word, i) => (
                                        <span key={i} className={`px-2 py-0.5 text-[8px] font-bold uppercase rounded-md ${foundForbidden.includes(word)
                                                ? 'bg-red-100 dark:bg-red-900/40 text-red-600 dark:text-red-400 ring-1 ring-red-300'
                                ...
                                {foundForbidden.length === 0 && (
                                    <p className="text-[10px] text-emerald-500 font-bold mt-2">✅ Tidak ada kata terlarang terdeteksi</p>
                                )}
                            </div>
                            ...
                            <div className="bg-white dark:bg-gray-800 rounded-[2rem] shadow-2xl overflow-hidden border border-gray-100 dark:border-gray-700/50 p-6 md:p-8">
                                <div className="flex items-center justify-between mb-8">
                                    <h3 className="text-xs font-black uppercase tracking-[0.2em] text-gray-400">Skor SEO</h3>
                                    {isAnalyzing && (
                                        <div className="flex gap-1">
                                            <span className="w-1.5 h-1.5 bg-indigo-500 rounded-full animate-bounce"></span>
                                            <span className="w-1.5 h-1.5 bg-indigo-500 rounded-full animate-bounce" style={{ animationDelay: '100ms' }}></span>
                                ...
                                <div className="space-y-4">
                                    <div className="flex items-end justify-between px-2">
                                        <span className={`text-6xl font-black ${seoScore >= 80 ? 'text-emerald-500' : (seoScore >= 50 ? 'text-amber-500' : 'text-red-500')}`}>{seoScore}</span>
                                    ...
                                    <div className="h-4 w-full bg-gray-100 dark:bg-gray-700/50 rounded-full overflow-hidden">
                                        <div className={`h-full transition-all duration-1000 ${getScoreColor(seoScore).split(' ')[1]}`} style={{ width: `${seoScore}%` }}></div>
                                    </div>
                                </div>
                                        ...
                                        {seoAnalysis ? Object.entries(seoAnalysis).map(([key, groupChecks]) => (
                                            <div key={key} className="space-y-3">
                                                <p className="text-[9px] font-bold text-gray-400 uppercase tracking-tighter">
                                                    {key === 'title' ? 'Judul' : key === 'meta' ? 'Meta' : key === 'content' ? 'Konten' : key === 'structure' ? 'Struktur' : key === 'links' ? 'Tautan' : key === 'media' ? 'Media' : key}
                                                </p>
                                                        ...
                                                        <li key={i} className="flex gap-3 group">
                                                            <div className="shrink-0 mt-0.5">
                                                                {check.status === 'success' ? (
                                                                    ...
                                                                    <div className="w-4 h-4 rounded-full bg-amber-100 dark:bg-amber-950/40 text-amber-600 flex items-center justify-center"><span className="text-[10px] font-black">!</span></div>
                                                                ) : (
                                ...
                                <div className="mt-10 p-5 bg-gradient-to-br from-indigo-600 to-indigo-800 rounded-3xl text-white shadow-xl shadow-indigo-600/20 relative overflow-hidden">
                                    ...
                                    <Link href="/admin/seo-settings" className="text-[9px] font-black uppercase bg-white/20 hover:bg-white/30 px-3 py-1.5 rounded-lg transition-all tracking-widest">
                                        Lihat Pengaturan SEO →
                                    </Link>
                                </div>
                            </div>
                        </div>
