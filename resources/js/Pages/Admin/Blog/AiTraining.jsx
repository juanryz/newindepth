...
export default function AiTraining({ instructions }) {
    ...
    return (
        <AuthenticatedLayout>
            <Head title="AI Training - Do's & Don'ts" />
            <div className="py-8 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
                {/* Header */}
                <div className="mb-8">
                    <div className="flex items-center gap-4 mb-2">
                        <Link href={route('admin.blog.create')} className="text-sm text-indigo-500 hover:text-indigo-700 font-medium">
                            ← Kembali ke Blog Editor
                        </Link>
                    </div>
                    ...
                    <p className="text-gray-500 dark:text-gray-400 mt-2 font-medium">
                        Ajarkan AI cara menulis konten blog yang sesuai dengan standar Anda. Tanpa coding — cukup ketik instruksi.
                    </p>
                </div>
                ...
                <div className="bg-gradient-to-br from-indigo-50 to-purple-50 dark:from-indigo-950/30 dark:to-purple-950/30 rounded-3xl p-6 mb-8 border border-indigo-200/50 dark:border-indigo-800/30">
                    <p className="text-sm font-bold text-indigo-700 dark:text-indigo-300 mb-2">💡 Cara Kerja</p>
                    <p className="text-sm text-gray-600 dark:text-gray-400 leading-relaxed">
                        Setiap instruksi yang Anda tambahkan di sini akan <strong>otomatis diinjeksi ke dalam prompt AI</strong> saat generate artikel.
                        Gunakan tab <strong>"DO"</strong> untuk hal yang harus dilakukan AI, dan tab <strong>"DON'T"</strong> untuk hal yang dilarang.
                    </p>
                </div>
                ...
                <div className="flex gap-3 mb-6">
                    <button
                        onClick={() => { setActiveTab('do'); setData('type', 'do'); }}
                        className={`px-6 py-3 rounded-2xl text-sm font-black uppercase tracking-widest transition-all ${activeTab === 'do'
                                ? 'bg-emerald-500 text-white shadow-lg shadow-emerald-500/30'
                                : 'bg-white dark:bg-gray-800 text-gray-500 dark:text-gray-400 border border-gray-200 dark:border-gray-700 hover:bg-emerald-50 dark:hover:bg-emerald-950/20'
                            }`}
                    >
                        ✅ DO ({instructions?.dos?.length || 0})
                    </button>
                    <button
                        onClick={() => { setActiveTab('dont'); setData('type', 'dont'); }}
                        className={`px-6 py-3 rounded-2xl text-sm font-black uppercase tracking-widest transition-all ${activeTab === 'dont'
                                ? 'bg-red-500 text-white shadow-lg shadow-red-500/30'
                                : 'bg-white dark:bg-gray-800 text-gray-500 dark:text-gray-400 border border-gray-200 dark:border-gray-700 hover:bg-red-50 dark:hover:bg-red-950/20'
                            }`}
                    >
                        🚫 DON'T ({instructions?.donts?.length || 0})
                    </button>
                </div>
                    ...
                    <div className="flex gap-3">
                        ...
                        <input
                            ...
                            placeholder={activeTab === 'do'
                                ? 'Contoh: Selalu sertakan data statistik dari sumber terpercaya'
                                : 'Contoh: Jangan gunakan bahasa yang terlalu teknis untuk audiens umum'
                            }
                        ...
                        <button
                            ...
                            className={`px-6 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest text-white shadow-lg transition-all disabled:opacity-50 ${activeTab === 'do'
                    ...
                    {currentItems.length === 0 ? (
                        <div className="text-center py-16 bg-white dark:bg-gray-800 rounded-3xl border border-dashed border-gray-300 dark:border-gray-700">
                            ...
                            <p className="text-gray-500 dark:text-gray-400 font-medium">
                                Belum ada instruksi {activeTab === 'do' ? 'DO' : "DON'T"}.
                            </p>
                            <p className="text-sm text-gray-400 dark:text-gray-500 mt-1">
                                Tambahkan instruksi di atas untuk mulai melatih AI Anda.
                            </p>
                        </div>
                    ) : (
                        currentItems.map((item, index) => {
                            ...
                            return (
                                <div
                                    key={item.id}
                                    className={`bg-white dark:bg-gray-800 rounded-2xl p-4 border transition-all ${item.is_active
                                ...
                                >
                                    <div className="flex items-center gap-4">
                                        {/* Number */}
                                        <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-black shrink-0 ${activeTab === 'do'
                                            ...
                                            {editingId === item.id ? (
                                                ...
                                                <>
                                                    <button
                                                        ...
                                                        className={`px-3 py-1 rounded-lg text-[10px] font-black uppercase ${item.is_active
                                                    ...
                                                    <button onClick={() => startEdit(item)} className="text-indigo-400 hover:text-indigo-600 p-1" title="Edit">
